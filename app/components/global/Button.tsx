// => Original Sizes
// ${size === 'lg' ? 'text-lz h-44 px-24' : size === 'md' ? 'h-36 px-16' : size === 'sm' ? 'h-28 px-12 text-xs' : ''}
...
/**
 * This is a type definition for a composite button component
 * that is used in the application.
 *
 * This type definition represents a button component that can be
 * rendered as an anchor element, a link element, or as a button
 * element, depending on the value of the renderAs property.
 *
 * The renderAs property is a union type that can be either "a",
 * "Link", or undefined. If the value of the renderAs property is
 * "a", then the button component will be rendered as an anchor
 * element. If the value of the renderAs property is "Link", then
 * the button component will be rendered as a link element. If
 * the value of the renderAs property is undefined, then the
 * button component will be rendered as a button element.
 *
 * If the button component is rendered as an anchor element,
 * then the anchor property will be set to the href attribute
 * value of the anchor element. If the button component is
 * rendered as a link element, then the link property will be
 * set to the href attribute value of the link element.
 * Otherwise, the anchor and link properties will be undefined.
 *
 * If the button component is rendered as a button element,
 * then the clicked property will be set to a function that
 * will be called when the button is clicked. Otherwise, the
 * clicked property will be undefined.
 */
    ...
    return (
      <>
        ...
        {!isDisabled &&
          hasSpinner &&
          (size == "sm" ? (
            <Spinner size={spinnerSize ?? "sm"} variant="primary" trackColor="white" />
          ) : size == "md" ? (
            <Spinner size={spinnerSize ?? "md"} variant="primary" trackColor="white" />
          ) : size == "lg" ? (
            <Spinner size={spinnerSize ?? "lg"} variant="primary" trackColor="white" />
          ) : (
            ""
          ))}
